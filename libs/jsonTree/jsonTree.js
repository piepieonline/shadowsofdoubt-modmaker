...
var jsonTree = (function() {
    /**
     * Rendering options. The CaseEditor had forked this vendored library to change
     * both of these; they are exposed here so one copy can serve both flows.
     * Set via jsonTree.configure({...}) before creating trees.
     */
    var config = {
        // Alphabetically sort object keys. The ScriptableObject flow turns this off
        // so fields render in the order the game serialises them.
        sortKeys: true,
        // Rendered after each non-final value. The ScriptableObject flow uses a
        // non-breaking space instead of a comma.
        valueSeparator: ','
    };
        ...
        forEachNode : function(obj, func) {
            ...
            switch (type) {
                ...
                case 'object':
                    ...
                    if (config.sortKeys) keys = keys.sort();
    ...
    function _NodeComplex(label, val, isLast) {
        ...
        var self = this,
            ...
            template = function(label, sym) {
                var comma = (!isLast) ? config.valueSeparator : '',
                    str = '\
