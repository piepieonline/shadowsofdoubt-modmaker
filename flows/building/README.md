...
troika's own four dependencies are mapped beside it rather than bundled, because a bundle
resolves `three` to a second copy of the same version — two module instances, and a `Text`
that is not the `Mesh` the rest of the scene is made of.

Everything the map names is vendored under `libs/vendor/`, generated by `npm run vendor`.
That includes the label font: troika would otherwise resolve one per codepoint range from
a CDN at runtime, which fails offline and is blocked outright by the desktop build's CSP.
`scene.js` pins it through `configureTextBuilder` to the same Noto Sans subset that
resolver was serving, so the glyphs and metrics are unchanged. Laying out text is still
asynchronous, so labels appear a moment after the floor does; nothing else in the view
waits for them.
